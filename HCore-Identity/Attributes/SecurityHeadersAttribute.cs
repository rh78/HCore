    ...
    public class SecurityHeadersAttribute : ActionFilterAttribute
    {
        ...
        public override void OnResultExecuting(ResultExecutingContext context)
        {
            ...
            if (result is ViewResult || result is PageResult || result is LocalRedirectResult)
            {
                ...
                var frameAncestorsAllowIFrameUrl = "";

                if (context.HttpContext.Items.ContainsKey(IdentityCoreConstants.AllowIFrameUrlContextKey))
                {
                    var allowIFrameUrl = (string)context.HttpContext.Items[IdentityCoreConstants.AllowIFrameUrlContextKey];

                    if (!string.IsNullOrEmpty(allowIFrameUrl))
                    {
                        frameAncestorsAllowIFrameUrl = $"{allowIFrameUrl}; ";
                    }
                }
                ...
                var csp = "default-src 'self' https://*.smint.io:40443 https://*.smint.io https://smintiocdn.azureedge.net; " +
                          "object-src 'none'; " +
                          $"frame-ancestors 'self' https://*.smint.io:40443 https://*.smint.io https://*.sharepoint.com https://*.officeapps.live.com https://*.veevavault.com; {frameAncestorsAllowIFrameUrl}" +
